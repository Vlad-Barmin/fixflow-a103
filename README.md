...
![Демо: заявка жильца → AI-классификация → карточка подрядчику → приём в работу](docs/demo.gif)

*Демо: заявка жильца → AI-классификация → карточка подрядчику → приём в работу*
...
| AI | Claude Sonnet 4.5 через OpenRouter (`anthropic/claude-sonnet-4.5`) |
...
Проект спроектирован и собран с AI-ассистированием — это осознанный подход, а не вынужденная мера.

Процесс: постановка задачи (`PROJECT_IDEA.md`) → техническая спецификация (`SPEC.md`) → автономная сборка по спецификации через Claude Code → ручная отладка, тестирование и деплой.

Правила проекта зафиксированы в `CLAUDE.md` — включая инженерные правила, выведенные из реальных инцидентов в ходе разработки (например, запрет fire-and-forget на serverless после того, как из-за него терялись результаты AI-классификации). Для code review и работы над AI-компонентом использованы специализированные субагенты (`.claude/agents/`).

Архитектурные решения, отладка, диагностика багов и вывод в продакшен — работа автора. AI в этом процессе — инструмент реализации, а не замена инженерного мышления.
...
- **Фоновая AI-классификация через `after()`** — запуск классификации после HTTP-ответа идёт через `after()` из `next/server`, что гарантирует завершение фоновой работы (в отличие от обычного fire-and-forget). При сбое классификации заявка не зависает молча, а уходит в `requires_manual_review` (graceful degradation)
...
По умолчанию активен реальный AI-режим (OpenRouter, `anthropic/claude-sonnet-4.5`). Keyword-fallback — запасной механизм на случай недоступности API или отсутствия ключа, а не временная заглушка.

### Режим 1: Claude Sonnet (полный AI, активен по умолчанию)

```
OPENROUTER_API_KEY=sk-or-...   # реальный платный ключ
USE_MOCK_CLASSIFIER=           # не задан или false
```

- Модель: `anthropic/claude-sonnet-4.5` через OpenRouter (OpenAI-совместимый `/chat/completions`)
...
Регулярные выражения по ключевым словам → 8 категорий без вызова API. Позволяет запустить систему без платного ключа и не терять заявки при недоступности OpenRouter API.
...
- Опционально: `OPENROUTER_API_KEY` для реальной AI-классификации
...
# OpenRouter (опционально — без него активен keyword-fallback)
...
| `OPENROUTER_API_KEY` | Нет | Ключ OpenRouter; без него активен keyword-fallback |
