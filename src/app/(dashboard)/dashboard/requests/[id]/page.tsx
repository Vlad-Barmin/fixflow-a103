  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <nav className="flex items-center gap-1 text-sm text-gray-500">
        <Link href="/dashboard" className="hover:text-gray-900">Дашборд</Link>
        <ChevronRight className="h-4 w-4" />
        <span className="text-gray-900 font-medium">Заявка #{req.id.slice(0, 8)}</span>
      </nav>
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <div className="flex items-center gap-3 flex-wrap">
            <h1 className="text-2xl font-bold text-gray-900">Заявка #{req.id.slice(0, 8)}</h1>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {aiRaw?.reasoning && (
                <div>
                  <p className="text-xs text-gray-500 mb-1">Обоснование AI</p>
          ...
          <Card>
            ...
            <CardContent>
              {contractor ? (
                ...
                <p className="text-sm text-gray-400 italic">Подрядчик не назначен</p>
              )}
            </CardContent>
          </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {photos.map((photo) => (
                    <div key={photo.id} className="aspect-square rounded-lg bg-gray-100 overflow-hidden border border-gray-200">
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <div>
                <p className="text-xs text-gray-500 mb-1">Текущий статус</p>
                <RequestStatusBadge status={req.status as RequestStatus} />
              </div>
              <Separator />
              <div>
                ...
                {req.deadline ? (
                  ...
                  <p className="text-sm text-gray-400">Не установлен</p>
                )}
              </div>
            </CardContent>
          </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                {apt.residential_complexes && (
                  <div>
                    <p className="text-xs text-gray-500">ЖК</p>
                    <p className="text-sm font-medium text-gray-900">{apt.residential_complexes.name}</p>
                    <p className="text-xs text-gray-400">{apt.residential_complexes.address}</p>
                  </div>
                )}
                <div className="flex gap-4">
                  <div>
                    <p className="text-xs text-gray-500">Корпус</p>
                    <p className="text-sm font-medium text-gray-900">{apt.building}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500">Квартира</p>
                    <p className="text-sm font-medium text-gray-900">{apt.number}</p>
                  </div>
                </div>
                {apt.owner_name && (
                  <div>
                    ...
                    <p className="text-sm text-gray-900">{apt.owner_name}</p>
                  </div>
                )}
                {apt.owner_phone && (
                  <div>
                    <p className="text-xs text-gray-500">Телефон</p>
                    <p className="text-sm text-gray-900">{apt.owner_phone}</p>
                  </div>
                )}
                {apt.warranty_expires_at && (
                  <div>
                    <p className="text-xs text-gray-500">Гарантия до</p>
